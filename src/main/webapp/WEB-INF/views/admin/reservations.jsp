...
<div class="max-w-6xl mx-auto px-4 py-10 fade-in">
    <h2 class="text-2xl font-bold mb-6" style="color:var(--text-primary);">📅 All Reservations</h2>
                    ...
                    <tbody>
                        <c:forEach var="r" items="${reservations}">
                            <tr style="border-top:1px solid var(--border-light);">
                                <td class="px-4 py-3" style="color:#b0a59a;">#${r.reservationId}</td>
                                <td class="px-4 py-3 font-medium" style="color:var(--text-primary);">${r.user.fullName}</td>
                                <td class="px-4 py-3" style="color:var(--text-secondary);">${r.table.tableNumber}</td>
                                <td class="px-4 py-3" style="color:var(--text-secondary);">${r.reservationDate}</td>
                                <td class="px-4 py-3" style="color:var(--text-secondary);">${r.reservationTime}</td>
                                <td class="px-4 py-3" style="color:var(--text-secondary);">${r.guestCount} pax</td>
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex gap-2">
                                        ...
                                        <c:if test="${r.status != 'CANCELLED' && r.status != 'COMPLETED'}">
                                            <form action="/reservations/admin/cancel/${r.reservationId}" method="post" onsubmit="return confirm('Cancel reservation #${r.reservationId}?')"><button type="submit" class="text-xs px-3 py-1.5 rounded-lg transition" style="background:rgba(220,38,38,0.05);color:#dc2626;border:1px solid rgba(220,38,38,0.12);">Cancel</button></form>
                                        </c:if>
                                    </div>
                                </td>
                            </tr>
